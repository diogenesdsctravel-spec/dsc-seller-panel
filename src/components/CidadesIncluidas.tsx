...
interface CidadesIncluidasProps {
  cities: CityInfo[];
  onBack: () => void;          // mantemos para navegação via AppPreview
  onVerRoteiro: () => void;
}

export function CidadesIncluidas({
  cities,
  onBack,
  onVerRoteiro,
}: CidadesIncluidasProps) {
  ...
  return (
    <div className="px-6 pt-16 pb-[120px]">
      ...
      >
        {/* não exibimos mais o botão Voltar aqui em cima, se não quiser é só remover este bloco */}
        {/* se quiser tirar completamente, apague este botão */}
        {/* 
        <button
          onClick={onBack}
